...
export function atomicSwap(args, issuerService, isInSession=false) {
  const {
    debug,
    COIN_RECOVERY,
    COIN_STORE,
  ...
  const defaultPolicy = this.getDefaultSettings().issuePolicy;
 ...
 *   swapInfo and the list of new coins from the targetCurrency.
 */
export function exportSwapCode(args, issuerService, tid = null) {
  const {
    COIN_RECOVERY,
    COIN_STORE,
    COIN_SWAP,
...
export function importSwapCode(request, toRemove, fee) {
  const {
    debug,
    COIN_STORE,
    DEFAULT_ISSUER,
...
export function includeSwapCoins(swapResponse, toRemove) {
  const {
    coin,
    swapInfo,
    verifyInfo,
...
export function revertSwapRequest (swapTransaction, reason) {
  const {
    debug,
    SESSION,
    COIN_STORE,
